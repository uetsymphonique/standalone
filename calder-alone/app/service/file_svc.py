...
class FileService(BaseService):
    ...
    async def walk_file_path(path, target):
        for root, _, files in os.walk(path):
            ...
            xored_target = FileService.add_xored_extension(target)
    ...
    def remove_xored_extension(filename):
        if FileService.is_extension_xored(filename):
    ...
    def _create_step(link, order):
        return {
            ...
            "order": order,
            "time-start": (link["delegated_timestamp"] or "0000-00-00T00:00:00Z").replace("Z",
                                                                                          ".000Z") \
                if "delegated_timestamp" in link else "0000-00-00T00:00:00.000Z",
            "time-stop": (link["finished_timestamp"] or "0000-00-00T00:00:00Z").replace("Z",
                                                                                        ".000Z") \
                if "finished_timestamp in link" else "0000-00-00T00:00:00.00Z",
            "output": [
                {
                    "content": link["output"]["stdout"] + link["output"]["stderr"] if "output" in link else "empty",
                    "level": "no info",
                    "type": "no info"
                }
            ]
